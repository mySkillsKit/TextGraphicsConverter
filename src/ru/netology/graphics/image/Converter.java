...
public class Converter implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        // если не заданы макс высота/ширина/Ratio то присвоить значение загруженной картинки
        if (width == 0 || height == 0) {
            width = img.getWidth();
            height = img.getHeight();
            maxRatio = (double) Math.max(img.getWidth(), img.getHeight())
                    / (double) Math.min(img.getWidth(), img.getHeight());
        }

        System.out.println("int width = " + width +
                "/ int height = " + height + " " +
                "/double maxRatio = " + maxRatio);
        ...
        System.out.println("Ширина/Высота картинки из интернета = "
                + img.getWidth() + " x " + img.getHeight() + " пиксель");


        double ratio = (double) Math.max(img.getWidth(), img.getHeight())
                / (double) Math.min(img.getWidth(), img.getHeight()); // 1280/853 = 1,5
        System.out.println("double ratio = " + ratio);
        ...
        if (img.getWidth() > width || img.getHeight() > height) {
            if (img.getHeight() > img.getWidth()) {     // 1280 > 853
                x = (double) img.getHeight() / (double) width;   // 1280/300 = 4,27
                //System.out.println(x);
                newWidth = (int) (img.getWidth() / x); //ширина 853 / 4,27 = 199
                newHeight = height;  //высота 1280 / 300 = 4,27
            } else {    //img.getHeight() < img.getWidth()
                x = (double) img.getWidth() / (double) height;
                newHeight = (int) (img.getHeight() / x);
                newWidth = width;
            }
        }
        ...
        ImageIO.write(bwImg, "png", new File("1bwImgOut.png"));
        ...
        System.out.println("Ширина/Высота картинки bwImg newWidth = "
                + bwImg.getWidth() + " x " + bwImg.getHeight() + " пиксель");
        ...
        ImageIO.write(bwImg, "png", new File("bwImgOut.png"));
        ...
        char[][] arrayChar = new char[bwImg.getWidth()][bwImg.getHeight()];

        fillArrayChar(arrayChar, schema, bwImg);
        ...
        return toString(arrayChar); // Возвращаем собранный текст.
    }

    private void fillArrayChar(char[][] arrayChar, TextColorSchema schema, BufferedImage bwImg) {
    ...
    private String toString(char[][] arrayChar) {

        StringBuilder sb = new StringBuilder();
        for (int h = 0; h < arrayChar[0].length; h++) {
            // Цикл по второй размерности выводит колонки - вывод одной строки
            for (int w = 0; w < arrayChar.length; w++) {
                //  без перехода на следующую строку
                sb.append(arrayChar[w][h] + "" + arrayChar[w][h]);
            }
